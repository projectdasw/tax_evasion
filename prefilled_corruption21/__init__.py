...
class C(BaseConstants):
    NAME_IN_URL = 'prefilled_corruption21'
    PLAYERS_PER_GROUP = 4
    NUM_ROUNDS = 1
...
def creating_session(subsession: Subsession):
    ...
    returntax = open(__name__ + '/parameter/higher_return.csv', encoding='utf-8-sig')
    ...
    for i in range(len(groups)):
        ...
        player = groups[i]
        player.return_tax = float(returntax_cols['105'])
...
class Group(BaseGroup):
    ...
    totalpajak = models.FloatField(min=0, max=1, initial=0)
    bagipajak = models.FloatField(min=0, max=1, initial=0)
...
class Player(BasePlayer):
    ...
    prefilled_form = models.FloatField(min=0, max=1, initial=0)
...
class FinalResults(Page):
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        if player.round_number == C.NUM_ROUNDS:
            ...
            participant.pay_treatment21 = player_in_selected_round.payoff
