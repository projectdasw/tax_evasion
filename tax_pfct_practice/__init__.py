...
class Player(BasePlayer):
    ...
    lat_pendapatan = models.FloatField(min=0, max=1, initial=0)
...
def set_jumlahpajak(group):
    ...
    for player in players:
        ...
        if group.corrupt > 0:
            group.lat_bagipajak = ((group.lat_totalpajak - (group.lat_totalpajak * (group.corrupt / 100))) *
                                   (group.return_tax / 100)) / Constants.players_per_group
        else:
            group.lat_bagipajak = (group.lat_totalpajak * (group.return_tax / 100)) / Constants.players_per_group
...
class Game(Page):
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        if player.num_correct > 0:
            player.lat_pendapatan += 100 * player.num_correct
        else:
            player.lat_pendapatan += 0
...
class BeforeTaxPage(Page):
    ...
    @staticmethod
    def before_next_page(player: Player, timeout_happened):
        player.lat_estimasi_pendapatan = player.lat_pendapatan * (player.prefilled_form / 100)
...
class TaxPage(Page):
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        laporpendapatanlebih = player.lat_laporpendapatan - player.lat_pendapatan
...
class PooledTax(Page):
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        pres_denda = (player.lat_pendapatan - player.lat_laporpendapatan) * 0.5
        laporpendapatanlebih = player.lat_laporpendapatan - player.lat_pendapatan

        if laporpendapatanlebih > 0 and player.audit == 1:
            player.denda = 0
            player.payoff = player.lat_pendapatan - player.lat_bebanpajak + hasilbagipajak
        elif player.audit == 1:
            player.denda = pres_denda
            player.payoff = player.lat_pendapatan - player.denda - player.lat_bebanpajak + hasilbagipajak
